		...
		for epoch in range(args.epoch_num):
			...
			if epoch % args.epoch_log == 0:
				...
				for id, data in enumerate(data_list):
					...
					prec_train += average_precision_score(label.flatten().cpu().numpy(), out_act(pred).flatten().data.cpu().numpy())
					recall_train += recall_score(label.flatten().cpu().numpy(), out_act(pred).flatten().data.cpu().numpy())
					f1_train += f1_score(label.flatten().cpu().numpy(), out_act(pred).flatten().data.cpu().numpy())
					# val
					...
					recall_val += recall_score(label.flatten().cpu().numpy(), out_act(pred).flatten().data.cpu().numpy())
					f1_val += f1_score(label.flatten().cpu().numpy(), out_act(pred).flatten().data.cpu().numpy())
					# test
					...
					recall_test += recall_score(label.flatten().cpu().numpy(), out_act(pred).flatten().data.cpu().numpy())
					f1_test += f1_score(label.flatten().cpu().numpy(), out_act(pred).flatten().data.cpu().numpy())
